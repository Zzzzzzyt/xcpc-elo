...
function buildContestRatingUpdates(eloData) {
  ...
  for (const player of players) {
    ...
    for (const event of history) {
      ...
      const ratingAfterContest = Number(event && event[4]);
...
function main() {
  ...
  const results = evaluation.results.sort(
    (left, right) =>
      right.pairAccuracy - left.pairAccuracy || left.rankMae - right.rankMae || left.mode.localeCompare(right.mode),
  );
