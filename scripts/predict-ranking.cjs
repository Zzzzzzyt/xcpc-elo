...
function main() {
  ...
  if (!inputArg) {
    throw new Error(
      `Usage: node scripts/predict-ranking.cjs <input.csv> [output.csv] [elo.json] [--mode ${AGGREGATION_MODES.join("|")}] [--verbose]`,
    );
  }
  ...
  const outputArg =
    outputArgFromCli ||
    (inputCsvPath.toLowerCase().endsWith(".csv")
      ? inputCsvPath.replace(/\.csv$/i, ".predicted.csv")
      : `${inputCsvPath}.predicted.csv`);
  ...
  const outputHeaders = ["predicted_rank", scoreHeader, ...parsedCsv.headers];
  if (verbose) {
  ...
  const outputRows = predictedRows.map((item) => {
    const row = [item.predictedRank, Math.round(item.ratingScore)];
    if (verbose) {
